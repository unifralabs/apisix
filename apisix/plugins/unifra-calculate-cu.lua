...
function _M.access(conf, ctx)
    ...
    -- Per-Method Cost Accounting (Array Strategy)
    -- Calculate cost for each method to ensure 100% accuracy in ClickHouse
    -- Logic: [cost1, cost2, ..., total_cost]
    -- The last item (total) corresponds to the "Summary Row" generated by ClickHouse arrayJoin if it's a batch
    local costs = {}
    ...
    -- Expose variables for Kafka Logger (ClickHouse Schema v2)
    ctx.var.total_cu_cost = total_cu
    ctx.var.cu_costs = cjson.encode(costs)

    core.log.info("total_cu_cost: ", total_cu, ", cu_costs: ", ctx.var.cu_costs)
end
