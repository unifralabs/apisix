...
```bash
# Copy to /opt (recommended for production)
sudo cp -r unifra-apisix /opt/unifra-apisix

# Or use Docker volume mount
docker run -v $(pwd)/unifra-apisix:/opt/unifra-apisix:ro apache/apisix:3.14.0-debian
```
...
```
unifra-apisix/
├── apisix/plugins/           # APISIX plugin wrappers
│   ├── unifra-jsonrpc-var.lua
│   ├── unifra-guard.lua
│   ├── unifra-ctx-var.lua
...
├── unifra/jsonrpc/           # Core business logic modules
│   ├── core.lua              # JSON-RPC parsing
│   ├── whitelist.lua         # Method access control
│   ├── cu.lua                # CU calculation
│   └── ratelimit.lua         # Redis rate limiting
├── conf/                     # Configuration files
│   ├── whitelist.json        # Method whitelist
│   ├── cu-pricing.json       # CU costs
│   └── config-example.yaml   # Example APISIX config
├── docs/                     # Documentation
│   ├── ARCHITECTURE.md
│   ├── PLUGINS.md
│   ├── DEPLOYMENT.md
